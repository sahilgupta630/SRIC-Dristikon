...
def import_project_list(excel_path, db_path):
    ...
    for idx, row in df.iterrows():
        ...
        # 3. Balance Record for Financials
        bal_record = AccProjectBalance(
            project_code=idx+1,
            balance_date=datetime.now().date(),
            op_balance=0.0,
            receipt_amount=sanctioned, # Assuming full amount received for simplicity
            payment_amount=0.0,
            cl_balance=sanctioned, # Available = Received (since no expense)
            deleted_flag='N',
